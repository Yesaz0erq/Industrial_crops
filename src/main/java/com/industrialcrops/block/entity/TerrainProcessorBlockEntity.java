...
public final class TerrainProcessorBlockEntity extends BlockEntity implements MenuProvider {
    ...
    public void resetDefaults() {
        ...
        startY = net.minecraft.util.Mth.clamp(worldPosition.getY()-1, level.getMinBuildHeight(), level.getMaxBuildHeight()-1);
    ...
    private boolean matchesSample(ItemStack stack) {
        return isBuildingMaterial(stack) && (items.getStackInSlot(0).isEmpty()
                || ItemStack.isSameItemSameTags(items.getStackInSlot(0),stack));
    }
    ...
    private boolean refill() {
        ...
        for (Direction side : Direction.values()) {
            ...
            var handler = com.industrialcrops.util.ForgeCapabilityUtil.find(level, ForgeCapabilities.ITEM_HANDLER,pos,side.getOpposite());
    ...
    private void exportDrops() {
        ...
        for (Direction side : Direction.values()) {
            ...
            var target = com.industrialcrops.util.ForgeCapabilityUtil.find(level, ForgeCapabilities.ITEM_HANDLER,pos,side.getOpposite());
    ...
    private Result process(ServerLevel server, BlockPos pos, FakePlayer player) {
        ...
        if (!old.isAir() && net.minecraftforge.common.MinecraftForge.EVENT_BUS.post(new BlockEvent.BreakEvent(server,pos,old,player))) {
        ...
        if (fill && ForgeEventFactory.onBlockPlace(player,snapshot,Direction.UP)) {
    ...
    @Override protected void saveAdditional(CompoundTag tag) {
        ...
        tag.put("Upgrades",upgrades.serializeNBT());tag.putBoolean("AutoSupply",autoSupply);
        tag.put("Items",items.serializeNBT());tag.putBoolean("Initialized",initialized);
