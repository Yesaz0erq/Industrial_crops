...
public final class AnimalBreederBlockEntity extends BlockEntity implements MenuProvider {
    ...
    public void configure(int count,boolean area,boolean on,boolean crops,boolean kill){
        limit=Math.clamp(count,2,4096);wide=area;enabled=on;harvest=crops;slaughter=kill;scanCursor=scanSection=0;setChanged();
    }
    ...
    private void refill(List<Animal> animals){
        ...
        for(Direction side:Direction.values()){
            ...
            IItemHandler source=level.getCapability(Capabilities.ItemHandler.BLOCK,pos,side.getOpposite());if(source==null)continue;
    ...
    private boolean breed(ServerLevel server,List<Animal> animals){
        ...
        for(int i=0;i<animals.size();i++){
            ...
            for(int j=i+1;j<animals.size();j++){
                ...
                var event=new net.neoforged.neoforge.event.entity.living.BabyEntitySpawnEvent(a,b,a.getBreedOffspring(server,b));
    ...
    public boolean harvestCrop(ServerLevel server,BlockPos pos,List<Animal> animals){
        ...
        if(NeoForge.EVENT_BUS.post(new net.neoforged.neoforge.event.level.BlockEvent.BreakEvent(server,pos,state,actor)).isCanceled())return false;
    ...
    @Override protected void saveAdditional(CompoundTag tag,HolderLookup.Provider lookup){super.saveAdditional(tag,lookup);
        tag.put("Items",items.serializeNBT(lookup));tag.put("Plasma",tank.writeToNBT(lookup,new CompoundTag()));tag.putInt("Energy",energy.getEnergyStored());
    ...
    @Override protected void loadAdditional(CompoundTag tag,HolderLookup.Provider lookup){super.loadAdditional(tag,lookup);
        if(tag.contains("Items"))items.deserializeNBT(lookup,tag.getCompound("Items"));if(tag.contains("Plasma"))tank.readFromNBT(lookup,tag.getCompound("Plasma"));energy.set(tag.getInt("Energy"));
    ...
    private final class Power extends EnergyStorage {
        Power(){super(ENERGY_CAPACITY,5000,0);}void set(int v){energy=Math.clamp(v,0,capacity);}void consume(int v){energy-=v;setChanged();}
