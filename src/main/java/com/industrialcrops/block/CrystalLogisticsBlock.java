...
public final class CrystalLogisticsBlock extends BaseEntityBlock {
    ...
    @Override public InteractionResult use(BlockState state, Level level, BlockPos pos, Player player, net.minecraft.world.InteractionHand hand, BlockHitResult hit) {
        if (level.getBlockEntity(pos) instanceof CrystalLogisticsBlockEntity machine
                && net.minecraftforge.fluids.FluidUtil.interactWithFluidHandler(player, hand, machine.fluidCapability()))
            return net.minecraft.world.InteractionResult.sidedSuccess(level.isClientSide);

        if (!level.isClientSide && player instanceof net.minecraft.server.level.ServerPlayer serverPlayer
                && level.getBlockEntity(pos) instanceof CrystalLogisticsBlockEntity machine) net.minecraftforge.network.NetworkHooks.openScreen((net.minecraft.server.level.ServerPlayer)player, machine, pos);
