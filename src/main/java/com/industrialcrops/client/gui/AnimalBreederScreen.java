...
public final class AnimalBreederScreen extends AbstractContainerScreen<AnimalBreederMenu> {
    ...
    private void send(){try{int n=Integer.parseInt(limit.getValue());if(n<2||n>4096)throw new NumberFormatException();PacketDistributor.sendToServer(new BreederConfigPayload(menu.containerId,n,wide,enabled,harvest,slaughter));error=false;}catch(NumberFormatException ex){error=true;}}
