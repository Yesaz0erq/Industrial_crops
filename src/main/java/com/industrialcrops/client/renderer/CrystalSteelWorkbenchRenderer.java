...
public final class CrystalSteelWorkbenchRenderer implements BlockEntityRenderer<CrystalSteelWorkbenchBlockEntity> {
    ...
    @Override public void render(CrystalSteelWorkbenchBlockEntity entity, float partialTick, PoseStack poses,
        ...
        float fraction = Math.clamp(fluid.getAmount() / (float) entity.getTank().getCapacity(), 0, 1);
        ...
        poses.translate(-0.5, 0, -0.5);
        var vertices = buffers.getBuffer(RenderType.entityCutoutNoCull(ResourceLocation.fromNamespaceAndPath("industrialcrops", "textures/block/workbench_gauge_blue.png")));
        ...
        var ticks = buffers.getBuffer(RenderType.entityCutoutNoCull(ResourceLocation.fromNamespaceAndPath(
    ...
    private static void vertex(VertexConsumer v, PoseStack.Pose pose, float x, float y, float u, float t,
        ...
        v.addVertex(pose, x, y, -0.001F).setColor(color).setUv(u, t).setOverlay(overlay)
