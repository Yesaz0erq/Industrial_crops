...
public final class CarroteCuriosEffects {
    ...
    public static ItemStack activeHelmet(LivingEntity entity) {
        ...
        if (worn.is(CarroteCuriosItems.HELMET)) return worn;
        if (entity instanceof Player player && ultimateActive(entity)) {
            return player.getInventory().items.stream().filter(s -> s.is(CarroteCuriosItems.HELMET)).findFirst().orElse(ItemStack.EMPTY);
        }
