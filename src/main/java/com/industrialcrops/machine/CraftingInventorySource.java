...
public final class CraftingInventorySource {
    ...
    private boolean addDrives(BlockEntity controller,boolean reinforced) {
        ...
        while(!queue.isEmpty() && visited.size()<4096 && slots.size()<8192) {
            ...
            for(var direction:net.minecraft.core.Direction.values()) {
                ...
                net.neoforged.neoforge.items.ItemStackHandler inventory=null;
    ...
    public boolean commit(List<ItemStack> items) {
        ...
        for (int i=0;i<items.size();i++) {
            ...
            if (!required.isEmpty() && (!ItemStack.isSameItemSameComponents(required,current)||current.getCount()<required.getCount())) return false;
        }
        ...
        for (int i=0;i<items.size();i++) {
            ...
            taken.add(extracted);
            if (!required.isEmpty() && (!ItemStack.isSameItemSameComponents(required,extracted)||required.getCount()!=extracted.getCount())) {
