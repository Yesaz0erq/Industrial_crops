...
public final class FluidPipeTransferUtil {
    ...
    public static boolean canConnectEndpoint(LevelAccessor level, BlockPos pos, @Nullable Direction side) {
        ...
        var blockEntity = realLevel.getBlockEntity(pos);
    ...
    private static List<Endpoint> collectEndpoints(Level level, BlockPos startPos) {
        ...
        for (EndpointKey key : keys) {
            var blockEntity = level.getBlockEntity(key.pos());
            if (blockEntity != null) blockEntity.getCapability(ForgeCapabilities.FLUID_HANDLER, key.side())
                    .ifPresent(handler -> result.add(new Endpoint(key.pos(), handler)));
        }
        return result;
    }
